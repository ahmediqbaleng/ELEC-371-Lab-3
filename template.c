...
void interrupt_handler(void)
{
   ...
   if ((ipending & 0x4000)  && (swit & 0x4)) { 
		*TIMER_STATUS = 0;
		
		if (counter==0){
			*LEDS = 0x380; //toggle leftmost LED
			counter++;
		}
		else if (counter == 1){
			*LEDS = 0x7; //toggle rightmost LED
			counter=0;
		}
...
void Init (void)
{
   ...
   *TIMER2_STATUS= 0x1; //assumption!!
   *TIMER2_CONTROL=7;

	//50 MHz = 20ns, 0.25s/20ns=100 000 000 = 0x05F5E100
   *TIMER3_START_LO=0xE100;
   *TIMER3_START_HI=0x05F5;
